    ...
    def bot_worker(self):
        ...
        try:
            while self.running:
                ...
                # 简化判定逻辑
                has_z = np.max(res_z) > THRESHOLD or (
                    np.sum((frame[:, :, 2] > 200) & (frame[:, :, 0] < 50)) > 50
                )
                has_x = np.max(res_x) > THRESHOLD or (
                    np.sum((frame[:, :, 1] > 100) & (frame[:, :, 2] < 90)) > 50
                )

                # 按键触发
                if has_z and not is_z:
                    pydirectinput.keyDown("z")
                    is_z = True
                elif not has_z and is_z:
                    pydirectinput.keyUp("z")
                    is_z = False

                if has_x and not is_x:
                    pydirectinput.keyDown("x")
                    is_x = True
                elif not has_x and is_x:
                    pydirectinput.keyUp("x")
                    is_x = False
        finally:
            camera.stop()
